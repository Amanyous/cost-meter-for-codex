 ...
 */
export async function queryCustomBalance(ctx, config, options = {}) {
  ...
  if (custom.adapter === 'aliyun') return queryAliyunBalance(ctx, config, options)
  ...
  const response = await fetchWithRetry(request.url, init, { timeoutMs: 15000, fetchImpl: options.fetchImpl })
