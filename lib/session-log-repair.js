...
async function canonicalLogPath(path) {
  ...
  for (let current = path; ; current = dirname(current)) {
    // macOS exposes stable root aliases such as /var -> /private/var.
    const allowedRootAlias = process.platform === 'darwin' && ['/etc', '/tmp', '/var'].includes(current)
    if (!allowedRootAlias && (await lstat(current)).isSymbolicLink()) throw new Error('不修复符号链接或目录链接中的日志')
