...
export function localDayKey(atMs, timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC') {
  ...
  const value = {}
  for (const part of formatter.formatToParts(new Date(atMs))) value[part.type] = part.value
...
export function createCostMeter(options = {}) {
  ...
  async function status(input = {}) {
    ...
    for (const { event, priced } of pricedEvents) {
      ...
      if (localDayKey(event.atMs, timeZone) === dayKey) {
        addUsage(today, event, priced)
      }

      const date = localDayKey(event.atMs, timeZone)
    ...
    let activeSession = input.threadId
      ? sessions.get(input.threadId) ?? { id: input.threadId, cwd: input.cwd || null, lastAtMs: now(), ...emptyBucket() }
      : null
    ...
    if (!lite) {
      ...
      .map(day => ({
        ...
        sessions: pricedEvents
          .filter(({ event }) => localDayKey(event.atMs, timeZone) === day.date)
    ...
    if (window) {
      ...
      for (const { event, priced } of pricedEvents) {
        const key = localDayKey(event.atMs, timeZone)
  ...
  return {
    ...
    setActiveThread: threadId => {
      if (!threadId || readActiveSession(dataDir)?.threadId === threadId) return
      writeActiveSession(dataDir, { threadId, atMs: Date.now() })
    },
    refreshActive,
    sync: () => syncFiles(),
