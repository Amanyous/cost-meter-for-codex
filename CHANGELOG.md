...
- 轮询改为轻量状态（不传输历史/模型大对象），进一步降低页面切换与刷新卡顿；峰谷倒计时移除秒。
- 零点后今日为 0 时仍只读轻量账本，等待 Stop hook 写入首笔当日 usage，避免自动解析大日志。
- 移除历史/模型页不必要的费用占比双横条，改为分类徽标和模型标签；Linux 悬浮窗恢复硬件加速。
- 简化历史/模型卡片为单条费用占比横条，分类改徽标说明；自动轮询改为只读账本，避免重复解析大日志。
- 峰谷轨道改用独立类名并加粗左橙右蓝渐变、时刻标记和进度百分比。
- 优化轮询性能：会话文件未变化时跳过重复解析，相同状态不再重建 DOM；峰谷轨道增加进度百分比标记。
- 历史与模型页大改：按天/模型费用横条对比、模型使用标签、API/Plan 分类条和模型汇总卡片。
- 紧凑预览加入原版风格峰谷横向轨道与当前时刻标记。
- DeepSeek 余额改为左对齐并增加原版风格余额横条；可配置余额上限用于进度比例。
- 面板改为响应式布局，拖动悬浮窗口时会自动重排卡片、状态行、表格和设置表单。
- 余额自动刷新默认改为 30 秒，并在设置页提供 15/30/60/120/300 秒选项。
- 余额超过 60 秒未更新时自动刷新，失败时保留最近一次成功余额。
- 悬浮预览底部峰谷时段、下一时段倒计时和 DeepSeek 余额改为高对比状态胶囊。
- 可见页面每 5 秒自动同步活动会话并刷新费用；设置页编辑时暂停轮询。
- 历史、模型、会话排行、额度和设置页统一为接近原版的卡片、表格和彩色强调样式。
- 优化概览视觉：费用卡片、预算、峰谷、额度和热力图加入接近原版的蓝色/橙色/绿色/紫色强调。
- 修复本会话费用误选最近小会话的问题：hook 现在记录活动 session，费用查询优先绑定当前任务。
- 修复 DeepSeek 官方余额在人民币显示模式下被重复汇率换算的问题；官方返回值现按原币种展示。
- 悬浮窗口展开改为放大窗口而非全屏；官方余额支持直接在本地设置中填写 Key。
- 紧凑预览卡新增当前峰谷时段、下一时段倒计时和 DeepSeek 官方剩余余额。
- 悬浮窗口增加可选峰谷切换系统通知；设置页补齐高级余额 JSON 配置入口。
- 余额与额度页补齐 Gateway 多账号、阿里云/千问余额、SCNet/Qwen 本地 Credits，以及设置页官方余额开关。
- 新增余额与额度页：DeepSeek 官方余额、自定义 Provider 余额、Coding Plan 查询与最近成功结果缓存。
- 新增预算金额/周期/自定义区间，以及 Plan provider/model 分类与 API/Plan 双轨金额统计。
- 新增 `cost-meter float`：使用 Electron 启动独立置顶悬浮窗口，复用同一 Web UI 与本地账本。
- 新增 Codex 插件适配：本地 MCP server、Stop/SubagentStop usage hooks、费用查询 skill、CLI 和 MCP Apps Web UI。
- Codex 按每个 turn 的实际 provider/model 计价，并以 `thread_id + response_id` 去重；DSH 原插件与账本保持独立。
- 新增 `SessionStart: startup` hook，新任务开始时自动渲染一次紧凑费用面板。
- 新增 repo marketplace、插件清单、UI 构建和 `npm run test:codex` 回归测试。

## [1.7.24] - 2026-09-14

- **#139 OpenCode Go 凭据发现**：读取 DSH `llm-pi-ai.providers` 中官方 Go 端点的 `apiKeyEnv`，支持 `opencodego` 等任意路由名。查询与配置状态使用同一候选顺序，保留专用 Key 优先、环境变量和 CLI 登录态回退；下一次刷新读取当前路由配置。
- 仅匹配 `https://opencode.ai/zen/go/v1`（可带尾斜线），不使用 Zen、代理或相似域名路由的凭据；设置和清除 Go Key 只操作专用引用。缺失提示补充模型路由检查路径，详见 [凭据说明](docs/opencode-go-credentials.md)。
- **#140 旧版会话恢复说明**：确认该报告中的 1.7.21 日志写入问题已由 1.7.22 修复；补充插件实际版本检查与桌面端升级说明。已有受影响日志仍需使用带备份的恢复工具。

## [1.7.23] - 2026-09-14

- **#137 自定义余额靠右显示**：侧栏普通余额行的金额与官方余额一样靠右对齐；长名称省略显示，悬停可查看全名，金额不被名称挤压。查询失败状态采用相同布局。
- 标准与简化侧栏、中英文、180px / 280px 布局已通过浏览器检查；现有全量回归与客户端构建门禁通过。
