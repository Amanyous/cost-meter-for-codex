...
## v1.7.24(2026-09-14)—— OpenCode Go 路由凭据自动发现

- 修复 #139：Go 额度读取官方 Go 路由实际配置的凭据引用，路由名无需固定；凭据配置状态同步修正。
- 专用 Go Key 优先，保留环境变量和 opencode 登录态回退。详见 [凭据发现说明](opencode-go-credentials.md)。
- #140 报告的是 1.7.21 已知日志问题，1.7.22 已修复。桌面程序升级后仍需确认插件实际版本；已有受影响会话见 [恢复说明](session-history-recovery.md)。

## v1.7.23(2026-09-14)—— 自定义余额靠右对齐

- 处理 #137：侧栏自定义余额金额靠右，与官方余额一致。长名称自动省略、悬停显示全名，查询失败状态保持相同对齐方式。
- 覆盖标准与简化侧栏、中英文及窄侧栏显示。
